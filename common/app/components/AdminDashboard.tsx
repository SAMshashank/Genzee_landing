...
export default function AdminDashboard() {
  ...
  const handleTextChange = (index: any, field:any, value: string,b:any) => {
  ...
  const handleStatusChange = (index: number, status: string) => {
    setPartnerData((prevData) =>
      prevData.map((partner, idx) =>
        idx === index ? { ...partner, status } : partner
      )
    );
  };
  // const handleTextChange = (field: string, subField: string, value: string, index: number) => {
  //   const updatedData = [...partnerData];
  //   updatedData[index].partnerUrl = value; // Update the URL for the correct partner
  //   setPartnerData(updatedData);
  // };

 const  handleImageUpload=(a:any,b:any)=>{
